import { ViewChild, ChangeDetectorRef, Component, signal, ChangeDetectionStrategy, DestroyRef, inject, OnInit } from '@angular/core';
...
export class HomeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.authService.getCurrentUser().pipe(
      ...
      if (user) {
        this.user = user;
        this.dataService.getUser(user.uid).pipe(
          take(1)
    ...
    this.loadTasks().pipe(
      ...
      error: (err) => this.logger.error('loadTasks error', err)
    });
  ...
  onSubmitPost(): void {
    ...
    if (content.length > 5000) {
      this.postCreationMessage = 'Post content must be 5000 characters or less.';
      this.postCreationSuccess = false;
      return;
    }
  ...
  loadPosts(): void {
    ...
    this.dataService.getPostsByRegion(this.userRegion).pipe(
      takeUntilDestroyed(this.destroyRef)
  ...
  getAuthorName(userId: string): string {
    ...
    this.dataService.getUser(userId).pipe(
      ...
      if (user) {
        this.authorNames.set(userId, user.displayName || user.uid);
      } else {
